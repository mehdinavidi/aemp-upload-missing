...
/* AEMP Pack-Demo – Single Packplatz + Hauptmenü + Bildverwaltung per Button
...
// Storage Keys
const KEY_SESSIONS = "aemp_demo_sessions_v5";
const KEY_USER = "aemp_demo_user_v3";
const KEY_ARCHIVE = "aemp_demo_archive_v2";
...
function hasAnyImage(obj){ return Boolean(imgThumb(obj) || imgFull(obj)); }
...
function renderSetList(filter = "") {
  ...
  results.forEach(s => {
    ...
    if (imgFull(s)) {
      ...
      img.addEventListener("click", (ev)=>{ ev.stopPropagation(); openLightbox(imgFull(s), `${s.code} – ${s.name}`); });
    }
...
// Upload Modal (manual)
function openUpload(target){ // target: {kind:'set'|'inst', id}
...
function deleteImage(target){
  const ov = loadImgOverrides();
  if (target.kind === 'set') {
    delete ov.sets[target.id];
  } else {
    delete ov.inst[target.id];
  }
  saveImgOverrides(ov);
  renderSetList(searchEl.value);
  renderDetails();
...
function renderDetails() {
  const view = document.getElementById("details");
  if (!selectedSetId) {
    view.innerHTML = '<div class="placeholder"><h2>Wähle links ein Set aus</h2><p>Dann siehst du hier die Details und kannst den Packvorgang starten.</p></div>';
    return;
  }
  ...
  view.innerHTML = `
    ...
    <div class="toolbar">
      ${
        hasAnyImage(s)
          ? `<img class="ithumb" src="${imgThumb(s)}" alt="${s.code}" style="width:120px;height:80px;cursor:${imgFull(s)?'zoom-in':'default'};border-radius:8px;border:1px solid var(--chip-border)"
               ${imgFull(s)?`data-zoom-src="${imgFull(s)}" data-caption="${s.code} – ${s.name}"`:''}/>`
  ...
  // Lightbox clicks
  view.querySelectorAll("[data-zoom-src]").forEach(el => {
  ...
  view.querySelectorAll("button[data-action='inst-upload']").forEach(b => {
    b.addEventListener("click", () => openUpload({kind:'inst', id: parseInt(b.dataset.id,10)}));
  });
  view.querySelectorAll("button[data-action='inst-delete']").forEach(b => {
    b.addEventListener("click", () => { if (confirm("Instrument-Bild wirklich löschen?")) deleteImage({kind:'inst', id: parseInt(b.dataset.id,10)}); });
  });
...
function openReport(setId) {
  ...
  const rows = sess.lines.map(l => `
    <tr><td>${l.instrument_name}</td><td>${l.qty_required}</td><td>${l.qty_found}</td>
    <td>${(l.qty_found<l.qty_required||l.missing) ? "Ja" : "Nein"}</td><td>${l.reason || ""}</td><td>${l.note || ""}</td></tr>
  `).join("");
  reportBody.innerHTML = `
    ...
    <table><thead><tr><th>Instrument</th><th>Soll</th><th>Ist</th><th>Fehlteil</th><th>Grund</th><th>Notiz</th></tr></thead>
...
// Steri-Freigabe: disabled placeholder – später implementieren
