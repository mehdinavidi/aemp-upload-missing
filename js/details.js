...
function openLightbox(src, caption=""){ if(!src) return; lbImg.src=src; lbCaption.textContent=caption; lightboxBackdrop.classList.remove("hidden"); lightboxBackdrop.classList.add("show"); }
function closeLightbox(){ lightboxBackdrop.classList.remove("show"); lightboxBackdrop.classList.add("hidden"); lbImg.src=""; lbCaption.textContent=""; }
...
uploadSave.addEventListener("click", async ()=>{
  ...
  if (window.UPLOAD_ENDPOINT && window.GithubUpload){
    try{
      const r = await GithubUpload.upload(currentUploadTarget.kind, currentUploadTarget.id, file);
      full=r.url; thumb=r.url; ghPath=r.path; ghSha=r.sha||null;
    }catch(e){ console.warn(e); const dataUrl = await fileToDataURL(file); full=thumb=dataUrl; }
  } else {
    const dataUrl = await fileToDataURL(file); full=thumb=dataUrl;
  }
...
function renderDetails(){
  ...
  const rows = lines.map(l=>`
    <tr>
      <td>${(loadImgOverrides().inst[l.instrument.id]?.thumb) ? `<img class="ithumb" src="${loadImgOverrides().inst[l.instrument.id].thumb}" data-zoom-src="${loadImgOverrides().inst[l.instrument.id].full}" data-caption="${l.instrument.name}">` : '<span class="subtle">kein Bild</span>'}</td>
