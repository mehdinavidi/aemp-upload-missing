...
function openReport(setId){
  ...
  reportBody.innerHTML = `<div class="muted">Abgeschlossen: ${new Date(sess.closed_at).toLocaleString()}</div><div class="muted">Benutzer: ${sess.closed_by||sess.started_by||"-"}</div><table class="table"><thead><tr><th>Instrument</th><th>Soll</th><th>Ist</th><th>Fehlteil</th><th>Grund</th><th>Notiz</th></tr></thead><tbody>${rows}</tbody></table>`;
